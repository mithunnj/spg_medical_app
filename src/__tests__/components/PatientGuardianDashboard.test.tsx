...
// Mock patient data structure for testing
const patientData = {
  'patient-1': {
    name: 'Emma Johnson',
    age: 8,
...
describe('Patient Guardian Dashboard Component Logic', () => {
  beforeEach(() => {
  ...
  it('should have valid follow-up appointments', () => {
    ...
    // Patient 2 has 2 appointments
    expect(patientData['patient-2'].followUpAppointments).toHaveLength(2)
    expect(patientData['patient-2'].followUpAppointments[0].type).toBe('Asthma follow-up')
    expect(patientData['patient-2'].followUpAppointments[1].type).toBe('Respiratory function test')
    
    // Patient 3 has no appointments (waitlisted)
  ...
  it('should have valid status values', () => {
    const validStatuses = ['PENDING', 'APPROVED', 'WAITLISTED']
    Object.values(patientData).forEach(patient => {
  ...
  it('should have valid clinic status values', () => {
    const validClinicStatuses = ['PENDING', 'APPROVED', 'WAITLISTED']
    Object.values(patientData).forEach(patient => {
  ...
  it('should calculate days since request with 30 day cap', () => {
    const recentDate = '2025-01-27'
    const oldDate = '2024-01-01'
    
    const recentDays = calculateDaysSinceRequest(recentDate)
    const oldDays = calculateDaysSinceRequest(oldDate)
    
    expect(recentDays).toBeGreaterThanOrEqual(0)
    expect(recentDays).toBeLessThanOrEqual(30)
    expect(oldDays).toBe(30) // Should be capped at 30
  })

  it('should have all patients from the same family', () => {
    Object.values(patientData).forEach(patient => {
      expect(patient.name).toContain('Johnson')
    })
  })

  it('should have realistic diagnosis descriptions', () => {
    expect(patientData['patient-1'].diagnosis).toContain('recovery')
    expect(patientData['patient-2'].diagnosis).toContain('management')
    expect(patientData['patient-3'].diagnosis).toContain('management')
  })

  it('should have appropriate clinic names', () => {
    Object.values(patientData).forEach(patient => {
      const hasValidName = patient.clinic.name.includes('Clinic') || 
                          patient.clinic.name.includes('Center') || 
                          patient.clinic.name.includes('Associates')
      expect(hasValidName).toBe(true)
    })
  })

  it('should have appropriate hospital names', () => {
    Object.values(patientData).forEach(patient => {
      expect(patient.hospital.name).toContain('Hospital')
    })
  })
}) 